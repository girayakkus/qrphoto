    ...
    try {
      ...
      await Promise.all(
        gallery.map(async (p, i) => {
          ...
          const safeName = (p.uploader || 'misafir').replace(
            /[^a-z0-9ığüşöçİĞÜŞÖÇ]/gi,
            '_',
          )
      ...
      console.error(err)
      showToast(
        'ZIP oluşturulamadı, tarayıcı bazı fotoğrafları engellemiş olabilir.',
      )
    }
  }
    ...
    <div className="app">
      <div className="topbar">
        ...
        <button onClick={handleToggleView}>
          {view === 'upload' ? 'Organizatör' : '← Yükleme Sayfası'}
        </button>
      </div>
      ...
      {view === 'upload' ? (
        ...
        <GalleryView
          ...
          onDownloadOne={(p) =>
            downloadUrl(
              p.secure_url,
              `${p.uploader || 'misafir'}-${p.public_id}.${p.format}`,
            )
          }
  ...
  return (
    <>
      <div className="hero">
        ...
        >
          <defs>
            <linearGradient id="ringGrad" x1="0" y1="0" x2="1" y2="1">
              <stop offset="0%" stopColor="#E4CB7E" />
          ...
          />
          <path
            d="M50 16l1.8 4.2L56 22l-4.2 1.8L50 28l-1.8-4.2L44 22l4.2-1.8L50 16z"
            fill="url(#ringGrad)"
          />
        </svg>
        <p className="welcome-line">
          Giray &amp; Yasemin&apos;in Nişanına Hoş Geldin
        </p>
        <h1>
          Bugünün Anılarını
          <br />
        ...
        <p>
          Çektiğin en güzel kareleri aşağıdan yükle, biz hepsini tek yerde
          toplayıp saklayalım.
        </p>
      </div>
      <div className="card">
        ...
        >
          {uploading
            ? 'Yükleniyor…'
            : `Fotoğrafları Yükle (${selectedFiles.length})`}
        </button>
      </div>
    </>
  )
      ...
      {!loading && gallery.length === 0 && (
        <div className="empty-state">
          ...
          <div>
            ...
            <br />
            QR kod paylaşılınca burada görünecek.
          </div>
        </div>
      )}
      {!loading && gallery.length > 0 && (
        <div className="polaroid-grid">
          {gallery.map((p, i) => (
            <div
              className="polaroid"
              key={p.public_id}
              style={{ transform: `rotate(${((i % 5) - 2) * 1.1}deg)` }}
            >
